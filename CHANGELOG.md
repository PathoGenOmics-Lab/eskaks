...
- **Two SNPs in one codon are now scored as the codon the samples actually carry.** eskaks
  used to build every alternate codon by copying the REFERENCE codon and mutating one
  base, so a codon carrying two SNPs was classified twice against the reference and its
  joint change was never evaluated. Where the input has per-sample genotypes, the new
  `vcf_analysis::mnv` module enumerates the codons the cohort actually holds, from the
  carrier sets, and scores each against the reference with **Nei-Gojobori pathway
  averaging** (average over the orders the changes could have happened in, skipping any
  order whose intermediate codon is a stop), which is the convention the FASTA dN/dS path
  has always used for a multi-difference codon pair; a test compares the two over all
  4,096 codon pairs of three genetic codes so they cannot drift. The invariant
  `Nd + Sd = k` is kept: a two-nucleotide change contributes two, split over the two
  alleles that make it up, so the counts stay commensurate with the per-nucleotide site
  denominator and a gene's `Total_SNPs` is still its number of classified alleles. On the
  bundled `variants_multisample.vcf` this rewrites 20 of the 264 `--variants` rows,
  including `gene12` residue 50, where a `TTG` (Leu) with SNPs at its second and third
  bases was reported as a missense `L50W` plus a synonymous `L50L` while the genomes carry
  `TGA`, a **premature stop**. Pooled pN/pS moves from 0.446028 to 0.456844, and only
  genes carrying a shared codon move at all. An input with **no genotype columns** cannot
  do any of this and is unchanged in every digit: each ALT is still scored against the
  reference codon, and the allele-frequency bound still reports which codons that leaves
  in doubt.
- **`Codon_Change` joins `Codon_Shared` under `--shared-codons`**: the codon change behind
  each row (`CTT>TTA`, coding strand), so a multi-nucleotide change is legible on the row
  instead of having to be reconstructed from two of them.
- **`MNV_Excluded` in the McDonald-Kreitman table.** Fisher's exact test and the SFS bins
  need whole alleles in one class, and an allele inside a multi-nucleotide change splits
  between the two, so both are restricted to alleles that stand alone in their codon for
  every sample carrying them (an allele carried by 300 samples of whom 50 also carry a
  neighbour splits just as surely as one carried jointly by all of them), and the
  exclusions are counted rather than rounded into a cell they only mostly belong in:
  `Dn + Ds + Pn + Ps + MNV_Excluded` equals the gene's `Total_SNPs`. A gene whose 2x2 is
  emptied entirely by the exclusion still gets a row, so the omission is never silent.
- **Same-codon SNPs are now detected exactly, from the per-sample genotypes.** eskaks
  assumes haploid genotypes, which settles the question the tool used to guess at: if one
  sample carries two variants in a codon they are on the same molecule by definition, so
  no phasing and no BAM is involved. The parser and the `--vcf-list` merge now retain the
  set of samples carrying each ALT (a bitset over sample indices, `ceil(n/64) * 8` bytes
  per allele: 2.4 MiB for 1,000 samples by 20,000 sites, 31 MB for 5,000 by 50,000), and
  the same-codon check intersects those sets instead of applying the allele-frequency
  pigeonhole bound. That bound is a floor and was missing real cases: on the bundled
  `variants_multisample.vcf` it reached 8 of the 13 codons where a sample genuinely
  carries two alleles, and on a 1,000-sample synthetic cohort whose frequencies all sit
  below 0.25 it reached 0 of 1,698. Both answers are now exact, so a codon is also
  *cleared* when no sample carries two of its SNPs, which a bound can never do. The
  warning distinguishes the two bases, and the bound itself is unchanged and still used
  for an AF-only VCF, where it remains the only sound inference available. The carrier
  sets are what makes the joint scoring above possible at all.
- **`--shared-codons`: a `Codon_Shared` column on the `--variants` table.** `true` when a
  sample carrying that ALT also carries another SNP in the same codon, so the row's
  amino-acid change is the joint one; `false` when no sample does; `NA` when the input
  cannot say, which is also the only case where the row may be wrong. The mark is per
  allele, not per codon, so a variant that stands alone is not flagged alongside its
  neighbours. Opt-in and appended last, so the existing variants table keeps the exact
  layout its parsers were written against.
...
- **The per-codon recurrence scan decides `Cooccurring` from the genotypes, not the
  allele-frequency floor.** A codon whose SNPs one sample carries together is one
  mutational event, so testing it counts a single mutation several times. The floor
  reached 8 of the toy VCF's 13 such codons; the genotypes reach all 13, and they also
  clear a codon whose frequencies the floor would have flagged but whose SNPs sit in
  disjoint samples. On the toy data the family drops from 1,426 to 1,421 codons and theta
  from 140/9,131 to 134/9,106, so every `P_Recurrence` moves; the codon the previous
  release ranked first (gene06 residue 44, "S44A;S44W") turns out to be one change,
  `TCG->GGG` = `S44G`, and is no longer tested at all. An AF-only VCF keeps the bound.
- **The same-codon report is now INFO where the joint change was evaluated, and WARN only
  where it could not be.** Previously every observed co-occurrence warned that the counts
  might be wrong; for a genotyped input they are not, so the line states what was done
  and how many alleles it involved. The warnings that remain are the ones a reader must
  act on: codons that gain a premature stop only jointly, and codons an AF-only input
  leaves unevaluated.
