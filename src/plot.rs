...
pub fn histogram_svg(stats: &SummaryStats, path: &str) -> std::io::Result<()> {
    ...
    for i in 0..=num_y_ticks {
        ...
        let val = (max_count as f64 * frac) as usize;
        let _ = writeln!(svg,
            r#"<line x1="{}" y1="{}" x2="{}" y2="{}" class="grid"/>"#,
            MARGIN_LEFT, y, MARGIN_LEFT + PLOT_W, y);
        let _ = writeln!(svg,
            r#"<text x="{}" y="{}" class="tick-label" text-anchor="end" dominant-baseline="middle">{}</text>"#,
...
pub fn group_bar_svg(groups: &[GroupPlotData], path: &str) -> std::io::Result<()> {
    ...
    let max_y = groups.iter()
        .map(|g| if g.ci_high.is_finite() { g.ci_high } else { g.mean })
        .fold(f64::NEG_INFINITY, f64::max)
        .max(0.1) * 1.2;
    ...
    for (i, group) in groups.iter().enumerate() {
        ...
        let mean_clamped = group.mean.max(0.0);
        let bar_h = PLOT_H * (mean_clamped / max_y);
        let y = to_y(mean_clamped);
        let color = if group.mean < 1.0 { COLOR_PURIFYING } else { COLOR_POSITIVE };
